...
# JSON for this renderer to read. It was typed here and again in the site's
# TypeScript, and the two had diverged where nobody was looking: the site had
# REALITY's failure truncated to one sentence and neither of the two columns.
def _architecture() -> Mapping[str, object]:
    """The layers, stages, questions and rungs, from the exported document."""
    export = REPO_ROOT / "apps" / "site" / "src" / "content" / "architecture.json"
    if not export.is_file():
        raise SystemExit(f"REFUSED: no architecture at {export}; run mise run contracts:generate")
...
def _rows(key: str) -> list[dict[str, str]]:
    found = _architecture().get(key)
    return [row for row in (found if isinstance(found, list) else []) if isinstance(row, dict)]


def _strings(key: str) -> list[str]:
    found = _architecture().get(key)
    return [item for item in (found if isinstance(found, list) else []) if isinstance(item, str)]
...
def layer_rows() -> str:
    ...
    for L in _rows("layers"):
        ...
        <tr class="{cls}">
          ...
          <td class="lname">{e(L['name'])}<div class="q">{e(L['question'])}</div></td>
          <td class="fail">{e(L['failure'])}</td>
          <td class="guard mono">{e(L['guard'])}</td>
          <td class="ac">{e(L['as_code'])}</td>
          <td class="ad">{e(L['as_data'])}</td>
        </tr>""")
    return "".join(out)
...
def nested_blocks() -> str:
    return "".join(f"""
      <div class="nq">
        <div class="nqh"><span class="qbadge">{e(N['id'])}</span>{e(N['title'])}</div>
        <div class="nqc"><b>Contrast:</b> {e(N['contrast'])}</div>
        <div class="nqn"><b>If the contrast ties:</b> {e(N['if_it_ties'])}</div>
      </div>""" for N in _rows("questions"))
...
def flow_svg() -> str:
    ...
    for i, S in enumerate(_rows("stages")):
        y = TOP + i * (BH + GAP)
        gate = S.get("gate")
        ...
        o.append(f'<text x="{BX+10}" y="{y+11.5}" font-size="9.6" font-weight="700" '
                 f'fill="{RED if gate else INK}">{e(S["title"])}</text>')
        if gate:
        ...
        o.append(f'<text x="{BX+10}" y="{y+21}" font-size="7.5" fill="{MUT}" '
                 f'font-family="DejaVuSansMono,monospace">{e(S["source"])}</text>')
        o.append(f'<text x="{BX+10}" y="{y+30.5}" font-size="7.5" fill="{IND}" '
                 f'font-weight="700">\u2023 {e(S["as_code"])}</text>')
        o.append(f'<text x="{BX+190}" y="{y+30.5}" font-size="7.5" fill="{GRN}" '
                 f'font-weight="700">\u2023 {e(S["as_data"])}</text>')

        if i < len(_rows("stages")) - 1:
            cx = BX + BW / 2
  ...
  <div>
    ...
    <div class="scope">
      ...
      <div>
        ...
        <ul class="outl">{scope_items(_strings("scope_out"))}</ul>
      </div>
    </div>
  </div>
  <div>

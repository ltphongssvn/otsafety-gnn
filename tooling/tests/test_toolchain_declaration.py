...
def _toolchain() -> dict[str, Any]:
    raw = (REPO_ROOT / "toolchain.json").read_text(encoding="utf-8")
    declared: dict[str, Any] = json.loads(raw)
    return declared
...
def _tools() -> dict[str, Any]:
    return {
        name: entry
        for name, entry in _toolchain().items()
...
def test_every_artifact_carries_a_sha256_and_a_directory() -> None:
    for name, entry in _tools().items():
        for platform, artifact in entry["artifacts"].items():
            ...
            assert artifact.get("url", "").startswith("https://"), f"{name}/{platform} url"
            assert artifact.get("dir"), f"{name}/{platform} has no dir"
...
def test_every_tool_names_the_binaries_it_provides() -> None:
    for name, entry in _tools().items():
        assert entry.get("binaries"), f"{name} names no binaries"

...
def _railway() -> dict[str, Any]:
    toolchain: dict[str, Any] = json.loads((REPO_ROOT / "toolchain.json").read_text())
    assert "railway" in toolchain, "railway is not declared in toolchain.json"
    entry: dict[str, Any] = toolchain["railway"]
    return entry

...
class Handed(TypedDict):
    """What the spy records: four named slots, each read back by a test."""
...
class _Summary:
    """The summary the adapter writes to, recording what it was given."""
...
class _Run:
    """A W&B run that satisfies the adapter's port, recording what it receives.

    IT IMPLEMENTS _Run FROM tracking_wandb, so mypy checks this double against the
    same protocol the real library satisfies structurally. The version this
    replaces wrapped a live wandb run and delegated through __getattr__, which
    needed Any at every seam and checked nothing.
    """

    def __init__(
        self, real: tracking_wandb._Run, sink: dict[str, object], logged: list[_Artifact]
    ) -> None:
        self._real = real
        self.summary = _Summary(sink)
        self._logged = logged
    ...
    def log_artifact(self, artifact: _Artifact) -> _Artifact:
...
@pytest.fixture
def handed(monkeypatch: pytest.MonkeyPatch) -> Handed:
    """What the adapter handed W&B, recorded while the real offline run happens."""
    seen: Handed = {"config": {}, "summary": {}, "artifacts": [], "init": {}}
    real_init = wandb.init

    def spy(**kwargs: object) -> _Run:
        seen["init"] = dict(kwargs)
        config = kwargs.get("config")
        seen["config"] = dict(config) if isinstance(config, dict) else {}
        return _Run(real_init(**kwargs), seen["summary"], seen["artifacts"])
...
def test_the_reproducibility_facts_are_recorded(tmp_path: Path, handed: Handed) -> None:
...
def test_a_failed_run_is_recorded_as_failed(tmp_path: Path, handed: Handed) -> None:
...
def test_the_run_keeps_the_times_it_was_measured_with(tmp_path: Path, handed: Handed) -> None:
...
def test_artifact_digests_are_recorded(tmp_path: Path, handed: Handed) -> None:
...
def test_an_initialisation_failure_propagates(
    ...
    def refuse(*_: object, **__: object) -> Never:
...
def test_the_record_is_logged_as_a_versioned_artifact(tmp_path: Path, handed: Handed) -> None:
    _tracker(tmp_path).record(_run())
...
def test_the_artifact_carries_the_facts_that_identify_it(tmp_path: Path, handed: Handed) -> None:
